...
class Plotter:
    def __init__(self, root):
        ...
        self.text_centroid = self.ax.text(0.5, 0.9, '', transform=self.ax.transAxes, fontsize=10, ha='center')
        self.text_fft = self.ax.text(0.5, 0.85, '', transform=self.ax.transAxes, fontsize=10, ha='center')
        self.text_max_intensity = self.ax.text(0.5, 0.80, '', transform=self.ax.transAxes, fontsize=10, ha='center')
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.spec.configure(integration_time=500 * 100)  # 500ms积分时间
    ...
    def _big_ui_window(self):
        self.root = tk.Tk()     # 创建新的Tkinter窗口
        self.root.title("信号分析结果")
        window_width = 950
        window_height = 750
        self.root.geometry(f"{window_width}x{window_height}")

        '''初始化plotter类==(在init内初始化可能会弹出两个窗口？或者矛盾)'''
    ...
    def _capture_loop(self):
        ...
        min_intensity = np.min(self.signal_process.apply_bandpass_filter(0.0211, 0.0279))
        self.min_ifft.append(min_intensity)
        if len(self.min_ifft) > 1:
            self.delta_fft.append(self.min_ifft[-1]-self.min_ifft[0])

        max_intensity = self.signal_process.track_reflection_intensity(400, 500)    # 400~500波段
        self.delta_intensity.append(max_intensity)

        # 显示
        self.plotter.plotting(self.wavelengths, self.current_net_spec, temp_centroid_x, temp_centroid_y, min_intensity, max_intensity)
